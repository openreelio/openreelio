...
The schemas are draft-07 and are generated by `schemars`, which annotates
numbers with `"format": "double"`, `"format": "uint"` and friends. Those are not
draft-07 standard formats, so a strict validator rejects the schema itself
rather than the payload: under ajv 6, construct it as
`new Ajv({ unknownFormats: 'ignore' })` (ajv 8 ignores unknown formats by
default). The annotations are informational — the `type` beside them is what
