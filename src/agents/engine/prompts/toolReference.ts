...
- style_caption(captionId, fontSize?, fontFamily?, fontWeight?, bold?, italic?, underline?, color?, opacity?, backgroundColor?, backgroundPadding?, outlineColor?, outlineWidth?, shadowColor?, shadowOffsetX?, shadowOffsetY?, shadowBlur?, alignment?, lineHeight?, letterSpacing?, position?, xPercent?, yPercent?) → style caption metadata; position is preset (bottom=5% subtitle safe area, top=5% margin, center) or custom xPercent/yPercent (0-100, origin top-left); a preset margin is a gap to the nearest edge of the box so a wrapped caption grows toward frame centre, while a custom point is the box centre; preview and export share both anchors; keep it readable: high contrast (light text + dark outline), ~32-42 chars per line, min on-screen ~1.5s
- get_caption_style(sequenceId, trackId?, captionId?) → read the track default style/position plus an existing caption's style so new captions can match it
- transcription_status() → inspect local Whisper availability and installed models before automatic subtitles
- install_whisper_model(model?, force?) → install a local Whisper model after user approval; default install target is large-v3-turbo-q5_0
- auto_transcribe(assetId, language?, model?, provider?, async?) → SOURCE-asset analysis only. Returns SOURCE-relative times (0-based to the asset) that must NOT be passed directly as timeline caption times; pass them to add_captions_from_transcription only with a clipId so they can be mapped to timeline time. Language is auto-detected by default; pass \`language\` only as an optional override when the user explicitly states the spoken language.
