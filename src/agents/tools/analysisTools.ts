...
type ClipAnalysisMode = 'representative' | 'dense';

interface ClipAnalysisOptionsPayload {
...
interface ClipAnalysisQualityLike {
  status?: string;
  score?: number;
}

interface ClipAnalysisBundleLike {
  fingerprint: string;
  sequenceId: string;
  trackId: string;
  clipId: string;
...
interface ClipAnalysisResponseLike {
  source: 'cached' | 'generated';
  bundle: ClipAnalysisBundleLike;
}

type ClipPerceptionDetail = NonNullable<ClipPerceptionOptions['detail']>;

interface ClipSemanticObservationLike {
  sampleId?: string;
  timelineSec?: number;
  sourceSec?: number;
...
function readNestedOptions(args: Record<string, unknown>): Record<string, unknown> {
