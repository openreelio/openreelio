...
export function PlanPartRenderer({
  ...
  const requiresApproval = status === 'proposed' && plan.requiresApproval;
  ...
  return (
    ...
    >
      <button
        ...
        className="w-full flex items-start gap-2 px-3 py-2 text-left bg-surface-elevated hover:bg-surface-active transition-colors"
        aria-expanded={isExpanded}
      >
        <span className="text-xs text-text-tertiary mt-0.5">{isExpanded ? '\u25BC' : '\u25B6'}</span>
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2">
            ...
            {requiresApproval && (
              <span className="text-xs px-1.5 py-0.5 rounded border border-yellow-500/20 bg-yellow-500/10 text-yellow-400">
                approval
              </span>
            )}
          </div>
          <p className="text-sm text-text-primary mt-1">{plan.goal}</p>
        </div>
        <span className="text-xs text-text-tertiary mt-0.5">{plan.steps.length} steps</span>
      </button>

      {isExpanded && (
        <div className="px-3 py-2 space-y-1.5 border-t border-border-subtle">
      ...
      {requiresApproval && (onApprove || onReject) && (
