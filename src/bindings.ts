...
export const commands = {
   ...
   */
  async indexSourceReportChunks(
    assetId: string,
    chunks: ReportChunkDto[],
  ): Promise<Result<null, string>> {
    try {
      return {
        status: 'ok',
        data: await TAURI_INVOKE('index_source_report_chunks', { assetId, chunks }),
      };
    } catch (e) {
      return { status: 'error', error: e as any };
    }
   ...
   */
  async searchSourceReportChunks(
    query: ReportChunkSearchQueryDto,
  ): Promise<Result<ReportChunkSearchResponseDto, string>> {
    try {
   ...
   */
  async importDiarizationJson(
    assetId: string,
    inputPath: string,
  ): Promise<Result<DiarizationImportSummary, string>> {
    try {
      return {
        status: 'ok',
        data: await TAURI_INVOKE('import_diarization_json', { assetId, inputPath }),
      };
    } catch (e) {
      return { status: 'error', error: e as any };
    }
   ...
   */
  async runExternalDiarization(
    assetId: string,
    executable: string,
    args: string[],
  ): Promise<Result<ExternalDiarizationRunSummary, string>> {
    try {
      return {
        status: 'ok',
        data: await TAURI_INVOKE('run_external_diarization', { assetId, executable, args }),
      };
    } catch (e) {
      return { status: 'error', error: e as any };
    }
...
export type DiarizationImportSummary = {
  assetId: string;
...
export type ExternalDiarizationRunSummary = {
  assetId: string;
 ...
 */
export type ReportChunkSearchQueryDto = {
  query: string;
  assetIds: string[] | null;
  sections: string[] | null;
  limit: number | null;
  useSemantic: boolean | null;
};
/**
 * Response payload for indexed source report chunk search.
 */
export type ReportChunkSearchResponseDto = {
  results: ReportChunkSearchResultDto[];
  total: number;
 ...
 */
export type ReportChunkSearchResultDto = {
  chunkId: string;
  assetId: string;
  sectionType: string;
  sectionIndex: number;
