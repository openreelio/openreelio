...
pub mod cache;
/// Measures the pixel extent each burned-in caption cue actually occupies.
pub mod caption_measure;
...
pub use cache::{
    ...
    compute_window_content_hash, enforce_cache_limit, ensure_agent_render_range_within_cap,
    is_agent_render_output, is_cached_segment_name, load_manifest, manifest_for_profile,
    manifest_path, preview_profile_hash, profile_cache_dir, prune_agent_renders,
    prune_other_profile_caches, refresh_manifest_plan_fingerprints, refresh_manifest_segment_flags,
    render_cache_dir, resolve_cached_segment_path, save_manifest, segment_cache_file,
    sequence_cache_dir, CacheSegmentState, CacheSegmentStatusDto, InterruptedRenderPolicy,
    ManifestForProfile, RenderCacheConfig, RenderCacheManifest, RenderCacheSegment,
    RenderCacheStatus, SegmentFingerprint, SegmentFlagReason, AGENT_RENDER_IN_PROGRESS_GRACE_SEC,
    MAX_AGENT_RENDERS, MAX_AGENT_RENDER_RANGE_SEC, SEGMENT_FINGERPRINT_UNSET,
};
