...
///
/// `audioDurationSec` sits beside it when the file's sound outlasts its
/// pictures. `durationSec` is then the picture's length, which bounds a clip on
/// a video track, and this is the sound's, which bounds the linked audio clip
/// and any `--audio-only` insert. Null when the two agree, which is the usual
/// case.
///
/// `probeVersion` says which revision of the probe rules last measured the
/// file. Null means nothing has, or that the reading predates the marker, and
/// the next insert re-probes the file once.
