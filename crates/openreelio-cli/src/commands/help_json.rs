        ...
        "commands": {
            ...
            "frame.extract": {
                ...
                "params": {
                    ...
                    "file": { "type": "string", "required": false, "desc": "Rendered video file to extract from instead of the project timeline, using fast seeking in the file's own timebase. Works with --time, --times, and --grid (+ --between or --times). This is the cheap judging path: it sheets the artifact that was actually produced, so no per-cell timeline render happens and the frames match what 'verify --file' measured. Conflicts with --asset, --source-time, --sequence, and --mode. Times outside the file are rejected with its real duration in the message." },
                    ...
                    "between": { "type": "string", "required": false, "desc": "Range sampled by --grid, given as two values: START END. Requires --grid and conflicts with --times." },
                    "count": { "type": "number", "required": false, "desc": "Number of --between samples (default: columns * rows; must not exceed the grid capacity). Rows no sample reaches are dropped, so the reported rows can be fewer than --grid asked for. Meaningless with --times, which already fixes the cell count." },
