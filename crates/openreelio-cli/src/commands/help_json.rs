...
pub(crate) fn build_schema() -> serde_json::Value {
    ...
    serde_json::json!({
        ...
        "commands": {
            ...
            "asset.import": {
                "description": "Import a media file as a project asset. The file is probed with ffprobe by default, so the asset records durationSec, video {width,height,fps} and audio, and a clip inserted from it is as long as the media. durationSec is the PICTURE's length for a video; when the sound outlasts it, audioDurationSec carries the sound's length and bounds the linked audio clip and any --audio-only insert. 'probed' says whether the reading happened and 'warnings' says why it did not",
                "params": {
                    "path": { "type": "string", "required": true, "desc": "Project directory path" },
