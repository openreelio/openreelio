        ...
        "commands": {
            ...
            "frame.extract": {
                ...
                "params": {
                    ...
                    "grid": { "type": "string", "required": false, "desc": "Contact sheet layout as COLSxROWS (e.g. 3x2), at most 100 cells and at most 8000px on either finished edge (columns * cell width, rows * cell height); an oversized combination is rejected before any cell is extracted. Requires exactly one time source: a sampler, --between to sample a range evenly, or --times to place a specific list of moments. Pass 'auto' to let the layout follow the sample count (2 columns up to 2 samples, 3 up to 9, 4 up to 16, then 6); 'auto' needs a sampler or --times, since --between already fixes its own count." },
                    "at-cuts": { "type": "boolean", "required": false, "desc": "Sample both sides of every cut: the outgoing shot's last frame at cut - 1.5/fps (seeks resolve forward, so a smaller offset lands on the incoming shot) and the incoming shot's first frame at the cut itself. Reasons cutBefore and cutAfter. Cannot be combined with --time, --times, --between, --count, --asset or --file." },
                    "at-transitions": { "type": "boolean", "required": false, "desc": "Sample the start, the cut and the end of every two-input transition. Reasons transitionStart, transitionCut, transitionEnd. None of the three is a clip boundary, so they cannot be derived from 'timeline clips'." },
