...
use openreelio_core::assets::{needs_probe_refresh, AssetKind};
...
pub fn measure_asset(project: &ActiveProject, asset_id: &str) -> AssetMeasurement {
    ...
    // A recorded duration is no longer proof the asset was read under the rules
    // this build applies: `audioDurationSec` was added after projects had been
    // written, so a video imported before it bounds its linked audio clip by the
    // picture and cuts the last seconds of the recording off. The probe marker
    // is what separates such an asset from one whose sound genuinely does not
    // outlast its picture, so each stale asset is re-read once and then left
    // alone. See `Asset::probe_version`.
    let measured_duration = asset
        .duration_sec
        .is_some_and(|duration| duration.is_finite() && duration > 0.0);
    if measured_duration && !needs_probe_refresh(asset) {
        return AssetMeasurement {
            command: None,
            warnings: Vec::new(),
        };
    ...
    // A re-read that fails leaves an already-measured asset exactly as usable as
    // it was, and the insert it precedes is unaffected — so there is nothing for
    // the caller to act on and nothing to report. Only an asset with no length
    // at all is worth warning about, because that insert really does fall back
    // to the default.
...
pub fn guard_command_media_length(
    ...
    match payload {
        // Every payload that places a clip from an asset, not just the two an
        // agent reaches for most: an `OverwriteEdit` from an unprobed asset
        // took the ten-second default exactly as an `InsertClip` did.
        ...
        CommandPayload::InsertEdit(insert) => ensure_asset_measured(project, &insert.asset_id),
        CommandPayload::OverwriteEdit(overwrite) => {
            ensure_asset_measured(project, &overwrite.asset_id)
        }
